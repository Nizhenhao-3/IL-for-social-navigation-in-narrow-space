...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    # 保存预测动作
    pred_df = pd.DataFrame(pred_actions_all, columns=['linear_x', 'linear_y', 'angular_z'])
    pred_df.to_csv('pred_actions.csv', index=False)

    # 保存真实动作
    gt_df = pd.DataFrame(gt_actions_all, columns=['linear_x', 'linear_y', 'angular_z'])
    gt_df.to_csv('gt_actions.csv', index=False)
